...
export default class SetCategoryGoalsTool {
    ...
    async execute(input) {
        ...
        try {
            ...
            if (input.categoryId) {
                targetCategory = allCategories.find((cat) => cat.id === input.categoryId);
            }
            else if (input.categoryName) {
                // Try exact match first
                targetCategory = allCategories.find((cat) => cat.name.toLowerCase() === input.categoryName.toLowerCase());
                // If no exact match, try partial match
                if (!targetCategory) {
                    targetCategory = allCategories.find((cat) => cat.name.toLowerCase().includes(input.categoryName.toLowerCase()));
                }
            }
            if (!targetCategory) {
