...
class AutoDistributeFundsTool {
    ...
    async execute(input) {
        ...
        try {
            ...
            const distributionPlan = this.generateDistributionPlan(categories, amountToDistribute, input.strategy || "goals-first");
    ...
    generateDistributionPlan(categories, amountToDistribute, strategy) {
        const plan = [];
        let remainingAmount = amountToDistribute;
        if (strategy === "goals-first") {
            // Prioritize categories with underfunded goals
            const goalCategories = categories.filter(cat => cat.goal_type &&
                cat.goal_under_funded &&
                cat.goal_under_funded > 0).sort((a, b) => {
                // Sort by goal urgency (TBD > TB > MF > NEED)
                const goalPriority = { "TBD": 4, "TB": 3, "MF": 2, "NEED": 1 };
                const aPriority = goalPriority[a.goal_type] || 0;
                const bPriority = goalPriority[b.goal_type] || 0;
    ...
    async executeDistribution(budgetId, month, plan) {
